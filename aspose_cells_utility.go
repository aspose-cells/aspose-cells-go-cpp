...
func ConversionUtility_Convert_String_LoadOptions_String_SaveOptions(source string, loadoptions *LoadOptions, saveas string, saveoptions *SaveOptions)  error {
	...
	CGoReturnPtr := C.CellsGoFunctoinZZNJ(C.CString("ConversionUtility_Convert_String_LoadOptions_String_SaveOptions"),C.CString(source), loadoptions_ptr, C.CString(saveas), saveoptions_ptr)
	if CGoReturnPtr.error_no != 0 {
		err := errors.New(C.GoString(CGoReturnPtr.error_message))	
		return  err
	}
...
func JsonUtility_ImportData(json string, cells *Cells, row int32, column int32, option *JsonLayoutOptions)  ([]int32,  error)  {
	...
	CGoReturnPtr := C.CellsGoFunctoinZZNK(C.CString("JsonUtility_ImportData"),C.CString(json), cells_ptr, C.int(row), C.int(column), option_ptr)
	if CGoReturnPtr.error_no != 0 {
		err := errors.New(C.GoString(CGoReturnPtr.error_message))	
		return  nil, err
	}
...
func JsonUtility_ExportRangeToJson(range_ *Range, options *JsonSaveOptions)  (string,  error)  {
	...
	CGoReturnPtr := C.CellsGoFunctoinZZIL(C.CString("JsonUtility_ExportRangeToJson"),range__ptr, options_ptr)
	if CGoReturnPtr.error_no != 0 {
		err := errors.New(C.GoString(CGoReturnPtr.error_message))	
		return  "", err
	}
