...
func (instance *SensitivityLabelCollection) Add(id string, isenabled bool, methodtype SensitivityLabelAssignmentType, siteid string, marktype SensitivityLabelMarkType)  (int32,  error)  {
	...
	CGoReturnPtr := C.CellsGoFunctoinZZPK(C.CString("SensitivityLabelCollection_Add"), instance.ptr, C.CString(id), C.bool(isenabled), C.int( int32(methodtype)), C.CString(siteid), C.int( int32(marktype)))
	if CGoReturnPtr.error_no != 0 {
		err := errors.New(C.GoString(CGoReturnPtr.error_message))	
		return  0, err
	}
