...
func NewDigitalSignature_Stream_String_String_Date(rawdata []byte, password string, comments string, signtime time.Time) ( *DigitalSignature, error) {
	...
	CGoReturnPtr := C.CellsGoFunctoinZZPC(C.CString("New_DigitalSignature_Stream_String_String_Date"),unsafe.Pointer(&rawdata[0]), C.int( len(rawdata)), C.CString(password), C.CString(comments), time_signtime)
...
func NewDigitalSignature_String_String_String_Date(filename string, password string, comments string, signtime time.Time) ( *DigitalSignature, error) {
	...
	CGoReturnPtr := C.CellsGoFunctoinZZPD(C.CString("New_DigitalSignature_String_String_String_Date"),C.CString(filename), C.CString(password), C.CString(comments), time_signtime)
	C.Delete_GetDate( time_signtime)
