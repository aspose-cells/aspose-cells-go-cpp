...
func (instance *VbaModuleCollection) AddUserForm(name string, codes string, designerstorage []byte) (int32, error) {
	...
	CGoReturnPtr := C.CellsGoFunctoinZZPQ(C.CString("VbaModuleCollection_AddUserForm"), instance.ptr, C.CString(name), C.CString(codes), unsafe.Pointer(&designerstorage[0]), C.int(len(designerstorage)))
	if CGoReturnPtr.error_no != 0 {
		err := errors.New(C.GoString(CGoReturnPtr.error_message))
		return 0, err
	}
...
func (instance *VbaProjectReferenceCollection) AddControlRefrernce(name string, libid string, twiddledlibid string, extendedlibid string) (int32, error) {
	...
	CGoReturnPtr := C.CellsGoFunctoinZZPR(C.CString("VbaProjectReferenceCollection_AddControlRefrernce"), instance.ptr, C.CString(name), C.CString(libid), C.CString(twiddledlibid), C.CString(extendedlibid))
	if CGoReturnPtr.error_no != 0 {
		err := errors.New(C.GoString(CGoReturnPtr.error_message))
		return 0, err
	}
